...
class Prk_json extends CI_Controller
{
	...
	function jsondetail()
	{
		...
		if(!empty($reqTahun))
		{
			$statement .= " AND A.PRK_YEAR='".$reqTahun."'";
		}
	...
	function add()
	{
		...
		$reqDistric= $this->input->post("reqDistric");
		$reqAccount= $this->input->post("reqAccount");
		$reqProjectNo= $this->input->post("reqProjectNo");
		$reqProjectDesc= $this->input->post("reqProjectDesc");
		$reqPoNo= $this->input->post("reqPoNo");
		$reqValue= $this->input->post("reqValue");
		$reqApprovalDate= $this->input->post("reqApprovalDate");
		...
		$set->setField("PRK_YEAR_OLD", $reqTahunOld);
		$set->setField("COST_ON_ASSET", str_replace(',', '', $reqCost));
		$set->setField("COST_ON_ASSET_OLD", str_replace('.', '', $reqCostOld));

		$set->setField("DSTRCT_CODE", $reqDistric);
		$set->setField("ACCOUNT_CODE", $reqAccount);
		$set->setField("PROJECT_NO", $reqProjectNo);
		$set->setField("PROJ_DESC", $reqProjectDesc);
		$set->setField("PO_NO", $reqPoNo);

		$set->setField("VALUE_PAID", ValToNullDB(str_replace(',', '', $reqValue)));
		$set->setField("LAST_APPR_DATE", dateToDBCheck($reqApprovalDate));
		...
		if(empty($checkKode))
		{
			echo "xxx***Asset No ".$checkKode." tidak ditemukan pada data asset lccm";exit;	
		}
