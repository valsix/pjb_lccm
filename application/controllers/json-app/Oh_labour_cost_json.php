...
class oh_labour_cost_json extends CI_Controller
{
	...
	function json()
	{
		...
		// print_r($this->appblokunitid);exit;
		...
		if(!empty($reqOhType))
		{
			$statement.= " AND A.OH_TYPE = '".$reqOhType."'";
		}
	...
	function add()
	{
		...
		$reqOhTypeOld= $this->input->post("reqOhTypeOld");
		...
		$set->setField("OH_TYPE_OLD", $reqOhTypeOld);
		...
		if ($reqMode == "insert")
		{
			...
			$statement=" AND A.ASSETNUM =  '".$reqAssetNum."' AND A.OH_TYPE =  '".$reqOhType."'  ";
			$check = new OhLabourCost();
			$check->selectByParams(array(), -1, -1, $statement);
			// echo $check->query;exit;
			$check->firstRow();
			$checkKode= $check->getField("ASSETNUM");
			$checkOh= $check->getField("OH_TYPE");

			if(!empty($checkKode))
			{
				echo "xxx***Data Oh Labour Cost Asset Num ".$checkKode." dan Oh Type  ".$checkOh." sudah ada";exit;	
			}
