<? 
  ...
  class Asset_Lccm extends Entity{ 
    ...
    function selectByParamstree($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY trim(A.PARENT) ASC")
	{
		$str = "
		SELECT 
			trim(A.PARENT) PARENT,A.ASSETNUM,A.DESCRIPTION,A2.CAPITAL_DATE, A2.CAPITAL

		FROM M_ASSET A 
		LEFT JOIN M_ASSET_LCCM A1 ON trim(A1.ASSETNUM) = trim(A.ASSETNUM) 
		LEFT JOIN M_CAPITAL_LCCM A2 ON trim(A2.ASSETNUM) = trim(A.ASSETNUM) and A2.SITEID =A.SITEID
		WHERE 1=1
