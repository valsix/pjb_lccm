...
class libapproval
{
	...
	function viewnew($arrparam)
	{
		$CI =& get_instance();
		$CI->load->view('app/approval_new', $arrparam);
	}
	...
	function approvaldata($arrparam)
	{
		...
		if($ref_tabel == "outlining_assessment_detil")
		{
			$ref_tabel=$ref_tabel;
		}
		else
		...
		$statement= " AND C.LINK_MODUL = '".$ref_tabel."'";
		...
		$set->setField("LAST_ROLE_ID", ValToNullDB($appuserroleid));
		$set->setField("NEXT_ROLE_ID", ValToNullDB($nroleid));
	...
	function reject($kode,$tabel,$id,$status,$ref_id,$alasan,$reqwaktu="")
	{
		...
		if($set->insertapprdetail())
		{
			...
			$statement= " AND A.APPR_ID = '".$kode."' ";
			$roledetil= new Approval();
			$roledetil->selectapproval(array(), 1,-1, $statement);
			// echo $roledetil->query;exit;
			$roledetil->firstRow();
			$nroleid= $roledetil->getField("LAST_ROLE_ID");
	...
	function updateapproval($kode,$tabel,$id,$status,$ref_id,$status_app,$role_id)
	{
		...
		if($tabel=="outlining_assessment_detil")
		{
			$zx="";
			$set->setField("V_STATUS_1", $vapprstatus);
			$set->updatetableapprovenew();
		}
		else
		{
			$zx="";
