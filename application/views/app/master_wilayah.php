...
$arrtabledata= array(
    ...
    // , array("label"=>"Direktorat", "field"=> "DIREKTORAT_NAMA", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    ...
    , array("label"=>"fieldid", "field"=> "WILAYAH_ID", "display"=>"1",  "width"=>"", "colspan"=>"", "rowspan"=>"")
);
$set= new Crud();
...
<script type="text/javascript">
    ...
    var elselect='<select id="reqStatus"  class="statusgen" style="color: #000000;" ><option value="">Semua</option><option value="NULL">Aktif</option><option value="1">Tidak Aktif</option></select>';

    $('.table').on('init.dt', function() {
        $('.selectstatus ').html(elselect);
        $('#reqStatus').on('change', function() {
            reqStatus=$('#reqStatus').val();
            jsonurl= "json-app/wilayah_json/json?reqStatus="+reqStatus;
	...
	$("#btnAdd, #btnEdit").on("click", function () {
        ...
        varurl= "app/index/master_wilayah_add?reqId="+valinfoid;
    ...
    $("#btnLihat").on("click", function () {
        ...
        varurl= "app/index/master_wilayah_add?reqId="+valinfoid+"&reqLihat=1";
        document.location.href = varurl;
    });

    $('#btnImport').on('click', function () {
        openAdd("app/index/master_wilayah_import");
    });
    ...
    $('#btnDelete').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/wilayah_json/update_status/?reqId="+valinfoid+"&reqStatus="+reqStatus,
                    function(data){
    ...
    $('#btnDeleteNew').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/wilayah_json/delete/?reqId="+valinfoid,
                    function(data){
                        $.messager.alert('Info', data.PESAN, 'info');
                        valinfoid= "";
                        setCariInfo();
	...
	$('#btnCari').on('click', function () {
        ...
        jsonurl= "json-app/wilayah_json/json?reqPencarian="+reqPencarian+"&reqStatus="+reqStatus;
        datanewtable.DataTable().ajax.url(jsonurl).load();
	});
	...
	jQuery(document).ready(function() {
		var jsonurl= "json-app/wilayah_json/json";
