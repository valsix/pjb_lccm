...
$arrtabledata= array(
    ...
    , array("label"=>"Start Date", "field"=> "START_DATE", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Stop Date ", "field"=> "STOP_DATE", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Duration (Hours)", "field"=> "DURATION_HOURS", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Load Derating", "field"=> "LOAD_DERATING", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Loss Output Year", "field"=> "LO_YEAR", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Loss Output Status", "field"=> "STATUS", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")

    , array("label"=>"fieldid", "field"=> "STOP_DATE", "display"=>"1",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"fieldid", "field"=> "START_DATE", "display"=>"1",  "width"=>"", "colspan"=>"", "rowspan"=>"")
...
<div class="col-md-12">
    <div class="judul-halaman"> Data Loss Output</div>
...
<script type="text/javascript">
    ...
    var indexfieldstart= arrdata.length - 3;
    var indexfieldend= arrdata.length - 4;
    var valinfoid= valinfotahun= valinfostart= valinfoend='';
    ...
    $("#btnAdd, #btnEdit").on("click", function () {
        ...
        varurl= "app/index/loss_output_add?reqId="+valinfoid+"&reqTahun="+valinfotahun+"&reqStart="+valinfostart+"&reqEnd="+valinfoend;
    ...
    $("#btnLihat").on("click", function () {
        ...
        varurl= "app/index/loss_output_add?reqId="+valinfoid+"&reqLihat=1";
        document.location.href = varurl;
    });

    $('#btnImport').on('click', function () {
        openAdd("app/index/loss_output_import");
    });

    $('#btnKembali').on('click', function () {
        varurl= "app/index/total_output";
        document.location.href = varurl;
    });
    ...
    $('#btnDelete').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/loss_output_json/update_status/?reqId="+valinfoid+"&reqStatus="+reqStatus,
                    function(data){
     ...
     $('#btnDeleteNew').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/loss_output_json/delete?reqId="+valinfoid+"&reqTahun="+valinfotahun+"&reqStart="+valinfostart+"&reqEnd="+valinfoend,
                    function(data){
                        $.messager.alert('Info', data.PESAN, 'info');
                        valinfoid= "";
                        location.reload();
    ...
    $('#btnCari').on('click', function () {
        ...
        jsonurl= "json-app/loss_output_json/json?reqDistrikId="+reqDistrikId+"&reqBlokId="+reqBlokId+"&reqUnitMesinId="+reqUnitMesinId;
    ...
    jQuery(document).ready(function() {
        var jsonurl= "json-app/loss_output_json/jsondetail?reqTahun=<?=$reqTahun?>&reqGroupPm=<?=$reqGroupPm?>";
    ...
    $(document).ready(function() {
        ...
        $('#example tbody').on( 'click', 'tr', function () {
            if ( $(this).hasClass('selected') ) {
                ...
                fieldinfostart= arrdata[indexfieldstart]["field"];
                fieldinfoend= arrdata[indexfieldend]["field"];
                valinfoid= dataselected[fieldinfoid];
                valinfotahun= dataselected[fieldinfotahun];
                valinfostart= dataselected[fieldinfostart];
                valinfoend= dataselected[fieldinfoend];
