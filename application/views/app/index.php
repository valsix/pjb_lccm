...
// print_r($this->appdistrikblokunitmesinnama);exit;
$rolenama=$this->appdistrikblokunitmesinnama;
if(empty($this->appdistrikblokunitmesinnama))
{
    $rolenama=$this->appdistrikblokunitnama;
}
...
$statement= " AND (LINK_MODUL = '".$infolinkmodul."' or LINK_MODUL = '".str_replace("_detil", "", $infolinkmodul)."' or LINK_MODUL = 'total_".$infolinkmodul."' or LINK_MODUL ='".str_replace("loss_", "total_", $infolinkmodul)."')";
$set= new Approval();
...
$set= new Approval();
$set->selectmenu($appuserkodehak," AND MENU = 1 AND KODE_MODUL NOT IN ('0802', '0803','0703','0702') ");
// echo $set->query;exit;
$arrMenu=[];
while($set->nextRow())
{
...
<head>
    ...
    <script src="assets/jspreadsheet/jexcel.js"></script>
    <script src="assets/jspreadsheet/jsuites.js"></script>
    <link rel="stylesheet" href="assets/jspreadsheet/jsuites.css" type="text/css" />
    <link rel="stylesheet" href="assets/jspreadsheet/jexcel.css" type="text/css" />
    ...
    <div class="wrapper">
        <nav class="main-header navbar navbar-expand navbar-white navbar-light">
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item">
                    <a class="nav-link logout" style="background:#235A81; margin-top: 10px;" href="role/entitas?pgold=<?=$pg?>"><i class="fa fa-map-marker fa-xs"></i> <?=coalesce($rolenama, "ENTITAS BELUM DITENTUKAN")?></a>
                </li>
                &nbsp;
                &nbsp;
        ...
        <aside class="main-sidebar sidebar-dark-primary elevation-4 bg-blur">
            <a href="app/" class="brand-link" style="position: relative;">
                <img src="images/logo-pln-np.png" class="img-responsive"> 
            </a>
            ...
            <div class="sidebar">
                ...
                function getmenubyparent($infoid, $arrMenu, $infolinkmodul)
                {
                    ...
                    foreach ($arrayKey as $valkey => $indexkey) 
                    {
                        ...
                        $infoactive= "";
                        if($infolinkmodul == $infolink || 'total_'.$infolinkmodul == $infolink|| str_replace("loss_", "total_", $infolinkmodul)== $infolink || str_replace("_detil", "", $infolinkmodul) == $infolink){
                            $infoactive= "active";
                        }
               ...
               <nav class="mt-2" id="myDIV">
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                        ...
                        foreach ($arrayKey as $valkey => $indexkey) 
                        {
                            ...
                            $infonama= $arrMenu[$indexkey]["NAMA"];
                            $infolink= $arrMenu[$indexkey]["LINK_MODUL"];
                            // echo$infogroupmodul.'-'.$infodatagroupmodul;
                        ?>
                        <li class="nav-item  <?if($infogroupmodul==$infodatagroupmodul){echo  "menu-is-opening menu-open";}?>">
                            <a href="<?=$infolink?>" class="nav-link <?if($infogroupmodul==$infodatagroupmodul){echo  "active";}?>"><strong><?=$infonama?></strong>
