...
$arrtabledata= array(
    ...
    , array("label"=>"F", "field"=> "F", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    // , array("label"=>"FP1", "field"=> "FP1", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"STATUS", "field"=> "STATUS_INFO", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
   

    , array("label"=>"fieldid", "field"=> "ID", "display"=>"1",  "width"=>"", "colspan"=>"", "rowspan"=>"")
);
...
<div class="col-md-12">
    <div class="judul-halaman"> Data Inflasi</div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-4">
                            <select class="form-control jscaribasicmultiple" id="reqStatus" <?=$disabled?>  style="width:100%;" >
                                <option value="">Semua</option>
                                <option value="NULL">Sesuai BI</option>
                                <option value="1">Need Update</option>
                            </select>
                        </div> -->
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    var elselect='<select id="reqStatus" class="statusgen" style="color: #000000;" ><option value="">Semua</option><option value="1">Sesuai BI</option><option value="NULL">Need Update</option></select>';
	...
	$("#btnAdd, #btnEdit").on("click", function () {
        ...
        varurl= "iframe/index/m_inflasi_add?reqId="+valinfoid;
    ...
    $("#btnLihat").on("click", function () {
        ...
        varurl= "iframe/index/m_inflasi_add?reqId="+valinfoid+"&reqLihat=1";
        document.location.href = varurl;
    });

    $('#btnImport').on('click', function () {
        openAdd("iframe/index/inflasi_import");
    });
    ...
    $('#btnDelete').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/inflasi_json/update_status/?reqId="+valinfoid+"&reqStatus="+reqStatus,
                    function(data){
     ...
     $('#btnDeleteNew').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/inflasi_json/deletem/?reqId="+valinfoid,
                    function(data){
                        $.messager.alert('Info', data.PESAN, 'info');
                        valinfoid= "";
                        setCariInfo();
	...
	$('#btnCari').on('click', function () {
        ...
        jsonurl= "json-app/inflasi_json/json_m?reqPencarian="+reqPencarian+"&reqKode="+reqKode+"&reqStatus="+reqStatus;
        datanewtable.DataTable().ajax.url(jsonurl).load();
	});
